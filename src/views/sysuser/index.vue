  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="700px">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="推荐人">
              <el-input v-model="form.referrer_name" placeholder="请输入姓名、用户名、手机号查找" @input="chang"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
      ...
      <div class="small" v-show="show">
            <div v-for="(item,index) in arr" :key="index" @click="choice(item)">{{item.nick_name}}  ({{item.username}})</div>
          </div>
...
import { userList } from '@/api/profitconfig/profitconfig'
...
export default {
  ...
  methods: {
    ...
    chang() {
        if(this.form.referrer_name.length!='') {
            userList({keyword:this.form.referrer_name}).then(res=> {
                this.arr=res.list
    ...
    choice(item) {
      this.form.referrer_name=item.nick_name
      this.form.referrer=item.userid
      this.show=false
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.userId !== undefined) {
            ...
            if (this.form.referrer==null) {
              this.msgError('没有选中推荐人')
            }
